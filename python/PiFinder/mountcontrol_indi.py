...
class PiFinderIndiClient(PyIndi.BaseClient):
    ...
    #remove previous indi config file
    def remove_xml(self) :
        home_directory = Path.home()
        folder_path = home_directory.joinpath(".indi")
        try:
            for filename in os.listdir(folder_path):
                if filename!="profiles.db" :
                ...
                print(f'Failed to find {folder_path}. Reason: {e}')
...
class MountControlIndi(MountControlBase):
    ...
    def __init__(
        ...
        indi_port: int = 7624,
        target_tolerance_deg: float = 0.3,
    ):
